...
try {
  Write-Log "Starting C:\Scripts\BR-DataIntegrationAPIAtScale\BR-DataIntegrationAPIScheduler.ps1 -JobId $($jobid) -SessionId $($sessionid) ..."
  Start-Process "powershell.exe" -ArgumentList "C:\Scripts\BR-DataIntegrationAPIAtScale\BR-DataIntegrationAPIScheduler.ps1 -JobId $jobid -SessionId $sessionid" -WorkingDirectory "C:\Scripts\BR-DataIntegrationAPIAtScale"
}
catch {
