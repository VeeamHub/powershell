#TEST OS 	: 2016 TP 5 
#ISO		: VeeamAvailabilitySuite_9.0.0.9.iso
$source = "D:"
$licensefile = "C:\silent\eplus veeam_availability_suite_nfr_12_12.lic"
$update = "C:\silent\veeam_backup_9.0.0.1715.update2_setup.exe"
...
#Visual c++ 2010
start-process -filepath "$source\Redistr\x64\vcredist100_x64.exe"  -ArgumentList "/log $logdir\03vc.html /q /norestart"  -PassThru | Wait-Process
...
#if installed separate, also install CLR, Shared Management Objects, C++ 2010 and DB
...
start-process -filepath msiexec -ArgumentList "/i `"$source\EnterpriseManager\BackupWeb_x64.msi`" /l*v $logdir\14entmgr.txt /qn ACCEPTEULA=`"YES`" VBREM_LICENSE_FILE=`"$licensefile`" VBREM_SERVICE_USER=`"$fulluser`" VBREM_SERVICE_PASSWORD=`"$password`" VBREM_SQLSERVER_SERVER=`"$computername\VEEAMSQL2012`" " -PassThru | Wait-Process
