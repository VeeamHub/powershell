...
.DESCRIPTION
    This script is designed to be run on a VBR server that sends backups to a (Cloud Connect) Cloud Repository. It looks for backups that no longer tied to an active Backup Job and then filters the results depending on the parameters specified.
...
foreach ($backup in $backups){
    ...
    if ($backup.HasParent){
        ...
        if ($jobs.Id.Guid -notcontains ($backups | Where-Object {$_.Id.Guid -eq $backup.ParentBackupId.Guid}).JobId.Guid){
            Write-Verbose "Backup does not have an active Backup Job."
