...
$LogFile = "BR-UpdateJobByAttribute.log"
...
Function Get-ViAttributeVMs {
    param($Server, $Attribute, $Value)

    $result = @{}
    $vms = Get-VM -Server $Server | Where-Object { $_.CustomFields[$Attribute] -eq $Value }
    foreach($vm in $vms) {
        $attributes = ""
