...
$InitializationScript = {
    function PreReqs {
        try{
            Install-Module AWS.Tools.Installer | Out-Null
            Install-AWSToolsModule AWS.Tools.Ec2 | Out-Null
        } catch {
            Write-Error "There is an error with the AWS PowerShell installation"
        } 
    }
    ...
    foreach ($ProxyName in $ProxyPool) {
        ...
        foreach ($ProxyVM in $VBOAutomatedProxies) {
            ...
            if ($match) {
                ...
                $ProxyVMIPv4 = $ProxyVM.ProxyVMIPv4
            }
        }
        if ($null -ne $ProxyVMName) {

            # Check AWS Powershell Installation
            ...
            if ($ProxyMode -eq 'Auto') {
                ...
                # Check PowerState
                    ...
                    $vm = $null
                    $ec2Name = New-Object Amazon.EC2.Model.Filter -Property @{Name = "tag:Name"; Values = "$ProxyVMName"}
                    $vm = (Get-EC2Instance -Filter $ec2Name).Instances | Where-Object -FilterScript {$_.PrivateIpAddress -eq "$ProxyVMIPv4"}
                    If ($vm.State.Name.Value -eq 'running') {
                        $script:ProxiesRunning = $true
                    }
                }
            ...
            elseif ($ProxyMode -eq 'Primary') {
                ...
                # Check PowerState
                    ...
                    $ec2Name = New-Object Amazon.EC2.Model.Filter -Property @{Name = "tag:Name"; Values = "$ProxyVMName"}
                    $vm = (Get-EC2Instance -Filter $ec2Name).Instances | Where-Object -FilterScript {$_.PrivateIpAddress -eq "$ProxyVMIPv4"}
                    If ($vm.State.Name.Value -like 'stop*') {
                        $script:PrimaryOffline = $true
                    }                    
                }
        ...
        if ($null -ne $ProxyVMName) {
            $ScriptBlock = {
                ...
                try {
                    ...
                    "[$((Get-Date -Format "MM/dd/yyyy HH:mm:ss").ToString())]`tError`t[ProxyJob ($($proxyHostname))] Error with AWS PowerShell Installation" | Out-File -FilePath $OutputFile -Append -Force
                }
                ...
                $vm = (Get-EC2Instance -Filter $ec2Name).Instances | Where-Object -FilterScript {$_.PrivateIpAddress -eq "$ProxyVMIPv4"}
                If ($vm.State.Name.Value -like 'stop*') {
                    "[$((Get-Date -Format "MM/dd/yyyy HH:mm:ss").ToString())]`tInfo`t[ProxyJob ($($proxyHostname))] Starting $($ProxyVMName)" | Out-File -FilePath $OutputFile -Append -Force
                    try {
                        Start-EC2Instance -InstanceId $vm.InstanceID
                        "[$((Get-Date -Format "MM/dd/yyyy HH:mm:ss").ToString())]`tInfo`t[ProxyJob ($($proxyHostname))] Start-EC2Instance command completed" | Out-File -FilePath $OutputFile -Append -Force
                    }
                    catch {
                        "[$((Get-Date -Format "MM/dd/yyyy HH:mm:ss").ToString())]`tError`t[ProxyJob ($($proxyHostname))] Start-EC2Instance command failed" | Out-File -FilePath $OutputFile -Append -Force
                    }
            ...
            Start-Job -InitializationScript  $InitializationScript -Name $proxyHostname -ScriptBlock  $ScriptBlock -ArgumentList $OutputFile, $proxyHostname, $ProxyVMName, $ProxyVMIPv4, $ProxyUserName, $ProxyUserPass, $ProxyRootPass, $ProxyOS, $ProxyBootTime 
            if ($ProxyMode -eq 'Auto') {
                $script:ProxiesRunning = $true 
            }
        } 
        ...
        if ($null -ne $ProxyVMName) {
            ...
            $ScriptBlock = {
                ...
                $vm = $null
                    ...
                    $vm = (Get-EC2Instance -Filter $ec2Name).Instances | Where-Object -FilterScript {$_.PrivateIpAddress -eq "$ProxyVMIPv4"}
                If ($vm.State.Name.Value -eq 'running') {
                    "[$((Get-Date -Format "MM/dd/yyyy HH:mm:ss").ToString())]`tInfo`t[ProxyJob ($($proxyHostname))] Stopping $($ProxyVMName)" | Out-File -FilePath $OutputFile -Append -Force
                    try {
                        Stop-EC2Instance -InstanceId $vm.InstanceID
                        "[$((Get-Date -Format "MM/dd/yyyy HH:mm:ss").ToString())]`tInfo`t[ProxyJob ($($proxyHostname))] Stop-EC2Instance command completed" | Out-File -FilePath $OutputFile -Append -Force
                    }
                    catch {
                        "[$((Get-Date -Format "MM/dd/yyyy HH:mm:ss").ToString())]`tError`t[ProxyJob ($($proxyHostname))] Stop-EC2Instance command error occurred" | Out-File -FilePath $OutputFile -Append -Force
                    }
                }
            }
