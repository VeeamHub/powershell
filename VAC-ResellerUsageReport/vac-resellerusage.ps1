...
.OUTPUTS
	vac-resellerusage returns a PowerShell Object containing all data

.EXAMPLE
	vac-resellerusage.ps1 -Server "vac.contoso.local" -Username "contoso\jsmith" -Password "password"
...
.EXAMPLE
	vac-resellerusage.ps1 -Server "vac.contoso.local" -Credential (Get-Credential)
...
.EXAMPLE
	vac-resellerusage.ps1 -Server "vac.contoso.local" -Credential $cred_vac
...
.EXAMPLE
	vac-resellerusage.ps1 -Server "vac.contoso.local" -Username "contoso\jsmith" -Password "password" -Port 9999
...
.NOTES
	NAME:  vac-resellerusage.ps1
	VERSION: 1.0
	AUTHORS: Chris Arceneaux & Tim Hudson
...
Function Get-ResellerUsage{
	param([String] $ResellerId)

    # GET - /v2/resellers
	[String] $url = "https://" + $Server + ":" + $Port + "/v2/resellers/$ResellerID"
	Write-Verbose "VAC Reseller Usage Url: $url"
	...
	try {
		...
		Write-Error "`nERROR: Retrieving Reseller Usage Failed!"
		Exit 1
	}
	# End Reseller Usage
...
Function Get-managedClients{
	param([String] $ResellerId)

    # GET - /v2/resellers
	[String] $url = "https://" + $Server + ":" + $Port + "/v2/resellers/$ResellerID/managedClients"
	Write-Verbose "VAC Reseller Usage Url: $url"
	$headers = New-Object "System.Collections.Generic.Dictionary[[String],[String]]"
	$headers.Add("Authorization", "Bearer $token")
	try {
		$mcresponse = Invoke-RestMethod $url -Method 'GET' -Headers $headers -ErrorAction Stop
		return $mcresponse
	} catch {
		Write-Error "`nERROR: Retrieving Reseller Usage Failed!"
		Exit 1
...
# GET - /v2/resellers - Retrieve Resellers 
[String] $url = "https://" + $Server + ":" + $Port + "/v2/resellers"
Write-Verbose "Reseller Url: $url"
$headers = New-Object "System.Collections.Generic.Dictionary[[String],[String]]"
$headers.Add("Authorization", "Bearer $token")
try {
	$response = Invoke-RestMethod $url -Method 'GET' -Headers $headers -ErrorAction Stop
	...
	Write-Error "`nERROR: Retrieving Resellers Failed!"
	Exit 1
}
...
# Get Reseller Data
$detailedResellerUsage = @()
foreach ($reseller in $resellers){
	$managedClients = Get-managedClients -ResellerId $reseller.id
	$tenants = @()
	foreach ($client in $managedClients.tenantId){
		$tenants += $detailedUsage | Where-Object {$_.TenantId -eq $client}
	}

	#Adding up Veeam Backup & Replication for VMware/Hyper-V - VM
	$editionEnterprisePlusVBR = ($tenants | Where-Object {$_.VBR_VmBackups.Edition -eq "Enterprise Plus"}).VBR_VmBackups.UsedVms
	$editionEnterpriseVBR = ($tenants | Where-Object {$_.VBR_VmBackups.Edition -eq "Enterprise"}).VBR_VmBackups.UsedVms
	$editionStandardVBR = ($tenants | Where-Object {$_.VBR_VmBackups.Edition -eq "Standard"}).VBR_VmBackups.UsedVms

	# Adding up Veeam Agent for Windows/Linux - Server
	$vbrSrvAgent = ($tenants.VBR_WindowsSrvAgentUsedCount | Measure-Object -Sum).Sum
	$vacSrvAgent = ($tenants.VAC_WindowsSrvAgentUsedCount | Measure-Object -Sum).Sum
	$srvAgent = $vbrSrvAgent + $vacSrvAgent # Adding both VBR & VAC managed agent-based backups for total count
	$resellerObject = [PSCustomObject] @{
		# Basic Reseller Information
		Alarms = $alarms
		ResellerName = $reseller.name
		ResellerId = $reseller.id
		BackupUsage_GB = [math]::round($reseller.cloudBackupResourceUsage / 1Gb, 2)
		BackupQuota_GB = [math]::round($reseller.cloudBackupResource / 1Gb, 2) #convert bytes to GB
		...
		CC_WsBackup_Total = ($tenants.CC_WsBackupUsedCount | Measure-Object -Sum).Sum
		CC_VmBackup_Total = ($tenants.CC_VmBackupUsedCount | Measure-Object -Sum).Sum
		CC_SrvBackup_Total = ($tenants.CC_SrvBackupUsedCount | Measure-Object -Sum).Sum
		CC_VmReplica_Total = ($tenants.CC_VmReplicaUsedCount | Measure-Object -Sum).Sum
		Agent_Ws_Total = ($tenants.VBR_WindowsWsAgentUsedCount | Measure-Object -Sum).Sum
		Agent_Srv_Total = $srvAgent
	}
	$detailedResellerUsage += $resellerObject
}
# End - Get Reseller Usage

# Outputting PowerShell object
return $detailedResellerUsage