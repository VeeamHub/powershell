<#
.SYNOPSIS
    Exports an M365 user mailbox to PST

.DESCRIPTION
    The script is designed to be executed on a VB365 server as it leverages both VB365
    & Exchange Explorer PowerShell cmdlets. It's interactive so no parameters are required.
    Upon execution, the script polls for information which culminates in the specified user
    mailbox being exported to a PST file.

.OUTPUTS
    Export-UserMailbox returns string output to guide the user
...
.LINK
    https://helpcenter.veeam.com/docs/vbo365/powershell/veeam_psreference.html?ver=60
...
# welcome message
Clear-Host
Write-Host "Export-UserMailbox.ps1" -ForegroundColor Green
Write-Host "-----------------------"
