...
.OUTPUTS
Get-MgmtAgentWindows.ps1 downloads a VSPC management agent for the macOS operating system to the current folder and then returns a string containing the file path of the downloaded agent.
...
$token = $ApiKey
$filePath = Join-Path -Path $PSScriptRoot -ChildPath ("\VeeamManagementAgentMac_" + $OrgId + "." + $PackageType)
